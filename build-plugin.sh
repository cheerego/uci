...
function build_go() {
    ...
    docker run --rm \
        ...
        -w /app \
        ${BUILD_IMAGE} \
        sh -cx "go build -mod=vendor -ldflags \"-X 'main.BUILD_TIME=`date '+%Y-%m-%d %H:%M:%S'`' -X 'main.VERSION=`git rev-parse HEAD`'\" -o ./bin/${module} ${import_path}/cmd/plugins/${module}  && chmod +x ${cur_user}/bin"
}

function build_docker_image() {
    local module="$1"
    status "building docker image for ${module}..."
    cp ${cur_dir}/cmd/plugins/${module}/Dockerfile ${cur_dir}/bin/Dockerfile.${module}
...
case "$1" in
    git-clone-cow)
        ...
        ;;
    *)
        echo "Usage: $0 git-clone-cow | asdf"
        ;;
esac
