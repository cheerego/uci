...
func Subscribe(c echo.Context) error {
	...
	if watch == "true" {
		z.S().Infof("requestId %s client %s subscribe ing", rid, uuid)
		...
		defer func() {
			z.S().Infof("requestId %s runner %s subscribe canceling", rid, runer.String())
			shim.Watcher.Unsubscribe(uuid)
			z.S().Infof("requestId %s runnerr %s subscribe canceled", rid, runer.String())
		}()
		...
		z.S().Infof("requestId %s runner %s subscribe success", rid, runer.String())
		for {
			select {

			case str, ok := <-subscribe:
				if !ok {
					z.S().Infof("requestId %s runner %s chan not ok return", rid, runer.String())
					return nil
				}
				...
				c.Response().Flush()
				z.S().Infof("requestId %s runner %s subscribe write message", rid, runer.String())
			case <-c.Request().Context().Done():
				z.S().Infof("requestId %s runner %s request done", rid, runer.String())
				...
				//	z.S().Infof("requestId %s client %s request CloseNotifier", rid, name)
				//	return nil
			}
		}
	}
