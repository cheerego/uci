...
func (w WaitForDispatchingPhase) Exec(ctx context.Context, p *pipeline.Pipeline) error {
	...
	if time.Now().Sub(p.FirstDispatchedAt.In(time.Local)).Minutes() > 5 {
		...
		if err != nil {
			...
			log.Info("pipeline dispatching timeout", zap.String("pipeline", p.String()))
			return nil
		}
	}
	...
	log.Info("publishing start pipeline letter", zap.String("pipeline", p.String()), zap.Any("letter", l))
	dispatchErr := shim.Watcher.PublishAck(fmt.Sprintf("%d", p.RunnerId), l)
	if dispatchErr != nil {
		log.Info("publishing start pipeline letter err", zap.String("pipeline", p.String()), zap.String("error", dispatchErr.Error()))
		return dispatchErr
	}
	p.DispatchSucceedAt = ptr.Ptr(time.Now())
	p.Status = pipeline.DispatchSucceed
	log.Info(" pipeline letter WaitForDispatching -> DispatchSucceed", zap.String("pipeline", p.String()))
	_, err := service.Services.PipelineService.Update(ctx, p)
	return err
