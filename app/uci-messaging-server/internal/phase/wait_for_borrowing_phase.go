...
func (b *WaitForBorrowingPhase) Exec(ctx context.Context, p *pipeline.Pipeline) error {
	...
	if time.Now().Sub(p.FirstBorrowRunnerAt.In(time.Local)).Minutes() > 5 {
		...
		if err != nil {
			...
			log.Info("pipeline borrow uci-uci-runner timeout ", zap.String("pipeline", p.String()))
			return nil
		}
	}
	...
	if err != nil {
		...
		log.Info("pipeline borrow uci-uci-runner success, WaitForBorrowing -> WaitForDispatching", zap.String("pipeline", p.String()), zap.String("uci-uci-runner", borrowRunner.String()))
		return nil
	}
