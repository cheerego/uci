...
func (l *ListWatch) Ack(msg string) error {
	...
	if err != nil {
		log.Error("json.Unmarshal letterString err", zap.Error(err), zap.String("letterString", msg))
		return err
	}
	...
	log.Infof("ack ackId %s letter %s", le.AckId, msg)
	return nil
}
...
func (l *ListWatch) Listening(ctx context.Context) error {
	for {
		select {
		case <-ctx.Done():
			log.Info("list watch listening canceled")
			return uerror.ErrContextCanceledOrTimeout.WithStack()
		default:
			log.Info("list watch start ...")
			rid, err := l.Watching(ctx)
			if err != nil {
				log.Infof("%s list watch err, %v", rid, err)
...
func (l *ListWatch) Watching(ctx context.Context) (string, error) {
	...
	log.Info("list watch success! 🎉", zap.String("requestId", rid))

	defer func() {
		log.Info("list and watch end", zap.String("requestId", rid))
	}()
	...
	for {
		...
		if len(line) > 0 {
			log.Info("list watch watching receive message", zap.String("line", line), zap.String("requestId", rid))
