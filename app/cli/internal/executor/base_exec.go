...
func (o *Executor) Exec(letterString string) {
	...
	if err != nil {
		log.Error("json.Unmarshal letterString err", zap.Error(err), zap.String("letterString", letterString))
		return
	}

	log.Infof("ack ackId %s letter %s", l.AckId, letterString)

	switch l.Action {
	case letter.StartAction:
		payload, err := l.StartPipelinePayload()
		if err != nil {
			log.Error("parse start pipeline payload err", zap.Error(err), zap.Any("payload", l.Payload))
			return
		}
	...
	case letter.StopAction:
		...
		if err != nil {
			log.Error("parse stop pipeline payload err", zap.Error(err), zap.Any("payload", l.Payload))
			return
		}
		err = o.stopAction(payload)

	default:
		log.Error("无效的 action 类型", zap.String("letterString", letterString), zap.String("action", string(l.Action)))
	}
}
