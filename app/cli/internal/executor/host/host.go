...
func (h *HostExecutor) Start(stopCtx context.Context, payload *letter.StartPipelinePayload, raw io.WriteCloser) error {
	log.L().Info("flow ", zap.Any("flow", payload.Scripts))
	workspaceDir, err := h.PrepareWorkspace(payload)
	if err != nil {
		log.L().Error("prepare workspace err", zap.String("pipeline", payload.TaskName), zap.Error(err))
		return err
	}

	log.S().Infof("pipeline %s workspace %s", payload.TaskName, workspaceDir)

	return nil
	//return h.RunScript(stopCtx, NewRuntime(workspaceDir, payload.Envs, raw), &flower)
}

//func (h *HostExecutor) RunScript(ctx context.Context, runtime *Runtime, f *flow.FlowYaml) error {
